...
$result = file_get_contents('http://192.168.1.232/oauth/token', false, $context);

$message = "Unknown";

if (strlen($result) < 1){
	$message = "Failed to authenticate with Hubitat";
}
else{
	...
	$getEndpointsResponse = file_get_contents("http://192.168.1.232/apps/api/endpoints", false, $getEndpointsContext);

	$success = false;

	if (strlen($getEndpointsResponse) < 1){
		$message = "Failed to get endpoints from Hubitat";
	}
	else{
		...
		$message = "Successfully authenticated with Hubitat. Please restart FPPD to use.";
...
<br />
<p>
<a href="/plugin.php?plugin=fpp-Hubitat&page=Hubitat.php">Return to plugin settings</a>
</p>

<script type="text/javascript">
$( document ).ready(function() {
    SetRestartFlag(2);
	<? if ($success) { echo "location.href = '/plugin.php?plugin=fpp-Hubitat&page=Hubitat.php';"; }  ?>
});
</script>
