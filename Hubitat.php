...
if (strlen($clientId) > 0 && strlen($clientSecret) > 0){
    ...
    $goAuthUrl = "http://192.168.1.232/oauth/authorize?response_type=code&scope=app&redirect_uri=" . urlencode($authorizedUrl) . "&client_id=" . $clientId;
}
...
echo "<a href='" . $goAuthUrl . "'>Configure Hubitat Access</a>";
}
?>
</td>
...
if ($isConfigured){
?>
<p style="font-size: 16pt; font-weight: bold;">Hubitat commands should now be available throughout FPP. If they are not, try restarting FPPD.</p>
<h3>Test</h3>
<table>
<tr>
<th>Switch</th>
...
function testOn(name){
	url = '/api/command/' + encodeURIComponent('Hubitat Device On') + '/' + encodeURIComponent(name);
	$.get( url, function( data ) {
		// no op
	});
}
function testOff(name){
	url = '/api/command/' + encodeURIComponent('Hubitat Device Off') + '/' + encodeURIComponent(name);
	$.get( url, function( data ) {
		// no op
	});
}
function testRoutine(name){
	url = '/api/command/' + encodeURIComponent('Hubitat Routine') + '/' + encodeURIComponent(name);
	$.get( url, function( data ) {
		// no op
	});
}
